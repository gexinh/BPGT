...
class GMT():
    ...
    def load_data(self, filename, genes_col=1, pathway_col=0):
        ...
        #     data_list = gmt.readlines().decode('gb18030')

        with open(filename, encoding='utf-8') as gmt:
    ...
    def load_data_dict(self, filename):
        ...
        data_dict_list = []
        dict = {}
        with open(os.path.join(data_dir, filename), encoding='utf-8') as gmt:
            data_list = gmt.readlines()
    ...
    def write_dict_to_file(self, dict, filename):
        lines = []
        with open(filename, 'w', encoding='utf-8') as gmt:
            for k in dict:
