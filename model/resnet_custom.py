...
class Inception_extractor(nn.Module):
    def __init__(self, name='inception_v4', retrain=False, ckp_path=None):
        super().__init__()
        if name == 18:
            self.inception = timm.create_model('inception_v4', pretrained=True)
        else:
            raise(ValueError('Model name must be inception_v4, inception_v3, or xx.'))

        # load fine-tuned weight
        if retrain == True:
            self.inception.load_state_dict(torch.load(ckp_path))
