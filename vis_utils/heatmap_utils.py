...
def compute_from_patches(wsi_object, clam_pred=None, model=None, feature_extractor=None, batch_size=512,  
    ...
    # TODO: Wsi region use patch size to caculate
...
def compute_from_patches_for_ML(args, wsi_object, label_pred=None, model=None, feature_path=None, batch_size=512,  
    attn_save_path=None, ref_scores=None, feat_save_path=None, **wsi_kwargs):
    """
        used for     
    """    
    ...
    feature_set = torch.load(feature_path)
    ...
    for idx, (roi, coords) in enumerate(roi_loader):
        ...
        with torch.no_grad():
            ...
            features = feature_set[idx,:]
            features = features.to(device)
            # print(features.device)
            model.relocate()
            # print(next(model.parameters()).device)
            if attn_save_path is not None:
                # modify model as this form
                A = model(features, attention_only=True)
                

                if A.size(0) > 1: # multi-label situation
                    A = A[label_pred,:]

                if args.agg_method == "cor_mil":
                    pad_num = A.size()[-1] - len(features)
                    A = A[:,:-pad_num]
                    ...
                    for score_idx in range(len(A)):
                        """
                         A `percentileofscore` of, for example, 80% means that 80% of the
                            ...
                            len(ref_scores<A[score_idx])/len(ref_scores)
